...
const Service2 = () => {
  ...
  const [formData, setFormData] = useState({ name: '', detail: '', servicefee: '' });
  const token = localStorage.getItem('Token');

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/addservice', {
  ...
  const handleModalClose = () => {
    ...
    setShowModal(false);
    setFormData({ name: '', detail: '', servicefee: '' });
  };
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://localhost:5000/api/addservice/create',
        formData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDelete = async (serviceId) => {
    try {
      await axios.delete(`http://localhost:5000/api/addservice/${serviceId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="flex flex-wrap justify-center gap-8 mt-32 ml-24">
      {services.map((service, index) => (
        <div
          key={index}
          className="max-w-sm rounded-md overflow-hidden shadow-lg transition-transform transform hover:scale-105"
        >
          <div className="bg-gray-200 p-4 flex items-center justify-center text-5xl">
            {service.name === 'Trimming' && <FaDog />}
            {service.name === 'Washing' && <FaShower />}
